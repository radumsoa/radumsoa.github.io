...
var sensorsX = [500, 500, 232, 551, 547];
var sensorsY = [300, 100, 218, 424, 251];
...
function setup() {
  ...
  canv = createCanvas(1042, 382);
  canv.parent('planwrapper');
  img_floorplan = loadImage("floorplan1.png");
...
function draw() {
 ...
 if (datapoints1.length == 0 || datapoints3.length == 0){
    store_local();
 ...
 if (counter %20 == 0){
  ...
  sensor_vals_new[1] = datapoints2.pop();
  sensor_vals_new[2] = datapoints3.pop();
 ...
 img_width = 1042;//windowWidth;
 img_height = 382;//0.5*img_floorplan.height/img_floorplan.width*windowWidth;
 ...
 for (var i = 0; i < numSensors; i++){
  ...
  if (sensor_vals[i] < sensor_vals_new[i]){
    //console.log("increasing sensor_vals[i]");
    sensor_vals[i]+= (sensor_vals_new[i]-sensor_vals[i])/2;
  }
  else if (sensor_vals[i] > sensor_vals_new[i]){
    sensor_vals[i]-= (sensor_vals[i]-sensor_vals_new[i])/2;
  }
